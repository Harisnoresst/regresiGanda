...
model_full.fit(X_pred_scaled, df[Y_COL].values)


# ── HELPER: hitung metrik OLS dari subset data ──────────────────────────────
def compute_ols_metrics(df_sub):
    """
    Hitung R², Adj.R², RMSE, MAE, koefisien dari df_sub.
    Mengembalikan dict metrik + array yte/ypred agar grafik
    actual-vs-predicted dan residual PERSIS pakai data yang sama.
    """
    ...
    intercept_orig = m.intercept_ - np.dot(coefs_orig, sc.mean_)

    abs_c = np.abs(m.coef_)
    pct   = (abs_c / abs_c.sum() * 100).tolist() if abs_c.sum() > 0 else [33.3, 33.3, 33.4]
    ...
    return {
        ...
        "rmse":      round(rmse, 2),
        "mae":       round(mae, 2),
        "intercept": round(float(intercept_orig), 2),
        "coefs":     [round(float(c), 4) for c in coefs_orig],
        "pct":       [round(float(p), 1) for p in pct],
        "n":         n,
        "n_test":    int(n_te),
        # ← array mentah untuk grafik (tidak dikirim ke JSON frontend)
        "_yte":      y_te,
        "_ypred":    y_pred,
    }
...
def make_pls_scores(df_sub, is_dark=False):
    ...
    if len(df_sub) < 5:
        return fig_to_b64(plt.subplots(figsize=(4.5, 3.8), facecolor=bg_color)[0], bg_color)
    sc = StandardScaler()
    X_s = sc.fit_transform(df_sub[X_COLS].values)
    ...
    ax.set_title(f"PLS Scores Plot  (n={len(df_sub)})", color=text_color,
                 fontsize=10, fontweight="bold", pad=8)
...
def make_pls_loadings(df_sub, is_dark=False):
    ...
    ax.set_title("PLS Loadings Plot", color=text_color, fontsize=10, fontweight="bold", pad=8)
    ax.set_xlabel("PLS Component 1", color=mut_color, fontsize=9)
    ax.set_ylabel("PLS Component 2", color=mut_color, fontsize=9)
    ax.tick_params(colors=mut_color, labelsize=8)
    for spine in ax.spines.values(): spine.set_edgecolor(LINE_COLOR)
...
def make_scatter(df_sub, is_dark=False):
    ...
    for i, (col, clr, lbl) in enumerate(zip(X_COLS, colors, labels)):
        ...
        for spine in ax.spines.values(): spine.set_edgecolor(LINE_COLOR)
    fig.suptitle(f"Scatter Plot: X vs ISPU Max  (n={len(df_sub)})", color=text_color,
                 fontsize=12, fontweight="bold", y=1.02)
    fig.tight_layout(pad=1.4)
    return fig_to_b64(fig, bg_color)

def make_actual_vs_pred(yte, ypred, r2_val, is_dark=False):
    """Grafik Actual vs Predicted — pakai yte/ypred/r2 DARI compute_ols_metrics
    sehingga R² di judul IDENTIK dengan kartu metrik."""
    bg_color, text_color, mut_color = get_theme_colors(is_dark)
    LINE_COLOR = "#4B5563" if is_dark else "#e2e8f0"
    fig, ax = plt.subplots(figsize=(4.5, 3.8), facecolor=bg_color)
    ax.set_facecolor(bg_color)
    ...
    ax.plot(lim, lim, color=PERFECT_LINE, linewidth=1.5, linestyle="--", label="Perfect fit (Y=X)")
    ax.set_xlim(lim); ax.set_ylim(lim)
    ax.set_title(f"Actual vs Predicted  (R²={r2_val:.4f})", color=text_color,
                 fontsize=10, fontweight="bold", pad=8)
...
def make_residual(yte, ypred, n_total, is_dark=False):
    """Residual plot — pakai yte/ypred DARI compute_ols_metrics."""
    bg_color, text_color, mut_color = get_theme_colors(is_dark)
    LINE_COLOR = "#4B5563" if is_dark else "#e2e8f0"
    residuals = yte - ypred
    fig, ax = plt.subplots(figsize=(4.5, 3.8), facecolor=bg_color)
    ...
    ax.set_title(f"Residual Plot  (n_test={len(yte)} / n={n_total})", color=text_color,
                 fontsize=10, fontweight="bold", pad=8)
...
# ── Hitung metrik default: 1000 baris pertama ────────────────────────────────
df_default_1000 = df_raw.head(1000)
_dm_raw = compute_ols_metrics(df_default_1000)
_dm_raw.pop('_yte', None)
_dm_raw.pop('_ypred', None)
default_metrics = _dm_raw
...
/* ── METRIK BANNER ── */
.metrics-banner{background:var(--card);border-radius:24px;padding:1rem 2rem;margin-bottom:1rem;display:flex;align-items:center;gap:1rem;box-shadow:0 8px 30px var(--shadow);flex-wrap:wrap}
.metrics-banner .mb-label{font-size:0.8rem;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.05em;margin-right:auto}
.metrics-banner .mb-n{font-family:"JetBrains Mono",monospace;font-size:0.9rem;font-weight:700;color:var(--accent);background:rgba(255,103,29,0.12);padding:0.3rem 0.9rem;border-radius:99px}
.metrics-banner .mb-sync{font-size:0.8rem;color:var(--muted);font-weight:500}

.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1.5rem;margin-bottom:3.5rem}
.metric{background:var(--card);border:none;border-radius:24px;padding:1.8rem;box-shadow:0 8px 30px var(--shadow);text-align:center;transition:all 0.3s}
.metric.updating{opacity:0.5;transform:scale(0.97)}
.metric-label{font-size:0.8rem;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.05em}
.metric-value{font-size:2.2rem;font-weight:800;margin:0.5rem 0 0.2rem;color:var(--text);font-family:"JetBrains Mono",monospace;letter-spacing:-1px;transition:all 0.4s}
...
.eq-formula{font-family:"JetBrains Mono",monospace;font-size:clamp(1rem,2vw,1.15rem);color:var(--text);line-height:1.8;background:var(--bg);padding:1.5rem;border-radius:16px;border:1px solid var(--border);font-weight:500;transition:all 0.4s}
...
/* Animasi nilai metrik berubah */
@keyframes flashUpdate{0%{opacity:0;transform:scale(0.9)}100%{opacity:1;transform:scale(1)}}
.metric-value.flash{animation:flashUpdate 0.4s ease-out}

.modal-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);backdrop-filter:blur(5px);z-index:9999;display:none;align-items:center;justify-content:center;opacity:0;transition:opacity 0.3s ease}
.modal-overlay.active{display:flex;opacity:1}
.modal-content{background:var(--card);border-radius:24px;padding:2rem;width:95%;max-width:1000px;max-height:90vh;overflow-y:auto;box-shadow:0 20px 40px rgba(0,0,0,0.4);border:1px solid var(--border);position:relative;transform:translateY(20px);transition:transform 0.3s ease}
.modal-overlay.active .modal-content{transform:translateY(0)}
.modal-close{position:absolute;top:1.5rem;right:1.5rem;background:var(--pill-bg);border:none;color:var(--text);width:36px;height:36px;border-radius:50%;font-size:1.2rem;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.2s}
...
@media(max-width:800px){.charts-2{grid-template-columns:1fr}.wrap{padding:0 1rem 3rem}.nav-wrapper{padding:0 1rem}.nav-links{display:none}.hero h1{font-size:2.2rem}.filter-bar{border-radius:20px}.modal-grid{grid-template-columns:1fr}.modal-img-container img{position:static}}
</style>
</head>
<body>
...
<div class="nav-wrapper">
  <nav>
    ...
    <div class="nav-links">
      <a href="#model">Metrik OLS</a>
      <a href="#prediksi">Prediksi</a>
      <a href="#dataset">Dataset</a>
      <a href="#pls">Grafik PLS</a>
    </div>
    <div class="nav-actions">
      <button class="nav-btn" id="nav-n-btn">n = 1000</button>
...
<div class="wrap">
  ...
  <div class="metrics">
    <div class="metric">
      ...
      <div class="metric-value" id="m-r2">{{ "%.4f"|format(def_r2) }}</div>
      <div class="metric-desc">Koefisien determinasi</div>
    </div>
    <div class="metric">
      <div class="metric-label">Adj. R²</div>
      <div class="metric-value" id="m-r2adj">{{ "%.4f"|format(def_r2_adj) }}</div>
      <div class="metric-desc">Adjusted R-squared</div>
    </div>
    <div class="metric">
      <div class="metric-label">RMSE</div>
      <div class="metric-value" id="m-rmse" style="color:var(--accent)">{{ "{:,.2f}".format(def_rmse) }}</div>
      <div class="metric-desc">Root Mean Sq. Error</div>
    </div>
    <div class="metric">
      <div class="metric-label">MAE</div>
      <div class="metric-value" id="m-mae">{{ "{:,.2f}".format(def_mae) }}</div>
      <div class="metric-desc">Mean Absolute Error</div>
    </div>
  </div>

  <div class="eq-card" id="eq-card">
    <div class="eq-title">Model Regresi Linear: Y = b₀ + b₁X₁ + b₂X₂ + b₃X₃</div>
    <div class="eq-formula" id="eq-formula">
      <span class="var">ISPU Max</span> =
      <span class="coef" id="eq-intercept">{{ "{:,.2f}".format(def_intercept) }}</span> +
      (<span class="coef" id="eq-c0">{{ "{:,.4f}".format(def_coefs[0]) }}</span> × <span class="var">pm10</span>) +
      (<span class="coef" id="eq-c1">{{ "{:,.4f}".format(def_coefs[1]) }}</span> × <span class="var">o3</span>) +
      (<span class="coef" id="eq-c2">{{ "{:,.4f}".format(def_coefs[2]) }}</span> × <span class="var">co</span>)
    </div>
    <table class="coef-table">
      <thead><tr><th>Variabel</th><th>Koefisien</th><th>Kontribusi Relatif</th></tr></thead>
      <tbody id="coef-tbody">
        {% for v, c, p in def_coef_rows %}
        <tr>
          <td>{{ v }}</td>
          <td class="coef-val">{{ "{:,.4f}".format(c) }}</td>
          <td>
            <div class="pct-val">{{ "%.1f"|format(p) }}%</div>
            <div class="coef-bar" style="width:{{ p }}%"></div>
          </td>
        </tr>
        {% endfor %}
      </tbody>
  ...
  <div class="filter-bar">
    ...
    <select class="sel" id="row-sel" onchange="applyFilters()">
      ...
      <option value="1000" selected>1000 baris</option>
  ...
  <div class="charts-2">
    <div class="chart-card" id="card-heatmap" style="cursor:pointer" onclick="openModal('heatmap')">
      ...
      <div class="chart-label">1. Heatmap Korelasi Pearson</div>
    </div>
    <div class="chart-card" id="card-pls-scores" style="cursor:pointer" onclick="openModal('pls_scores')">
      <div class="chart-spinner"><div class="spin"></div> Memproses...</div>
      <div class="chart-label">2. PLS Scores Plot (Persebaran Data)</div>
    </div>
  </div>
  ...
  <div class="charts-2">
    <div class="chart-card" id="card-pls-loadings" style="cursor:pointer" onclick="openModal('pls_loadings')">
      <div class="chart-spinner"><div class="spin"></div> Memproses...</div>
      <div class="chart-label">3. PLS Loadings (Arah/Pengaruh Variabel X)</div>
    </div>
  ...
  <div class="charts-1">
    <div class="chart-card" id="card-scatter" style="cursor:pointer" onclick="openModal('scatter')">
      <div class="chart-spinner"><div class="spin"></div> Memproses...</div>
      <div class="chart-label">4. Hubungan X vs ISPU Max beserta Garis Regresi Linier</div>
    </div>
  </div>
  ...
  <div class="charts-2">
    <div class="chart-card" id="card-avp" style="cursor:pointer" onclick="openModal('avp')">
      <div class="chart-spinner"><div class="spin"></div> Memproses...</div>
      <div class="chart-label">5. Actual vs Predicted (OLS)</div>
    </div>
    <div class="chart-card" id="card-res" style="cursor:pointer" onclick="openModal('residual')">
      <div class="chart-spinner"><div class="spin"></div> Memproses...</div>
      <div class="chart-label">6. Residual Plot (Homoskedastisitas)</div>
    </div>
  </div>
...
<div class="modal-overlay" id="math-modal" onclick="closeModal(event)">
  <div class="modal-content" onclick="event.stopPropagation()">
    ...
    <div class="modal-grid">
      <div class="modal-img-container" id="modal-img-target"></div>
...
// ── UPDATE METRIK DI DOM ──────────────────────────────────────────────────
function updateMetricsDom(metrics) {
  animateValue(document.getElementById("m-r2"),    metrics.r2.toFixed(4));
  animateValue(document.getElementById("m-r2adj"), metrics.r2_adj.toFixed(4));
  animateValue(document.getElementById("m-rmse"),  metrics.rmse.toLocaleString("en-US", {minimumFractionDigits:2, maximumFractionDigits:2}));
  animateValue(document.getElementById("m-mae"),   metrics.mae.toLocaleString("en-US",  {minimumFractionDigits:2, maximumFractionDigits:2}));

  // update persamaan regresi
  document.getElementById("eq-intercept").textContent = metrics.intercept.toLocaleString("en-US", {minimumFractionDigits:2, maximumFractionDigits:2});
  document.getElementById("eq-c0").textContent = metrics.coefs[0].toFixed(4);
  document.getElementById("eq-c1").textContent = metrics.coefs[1].toFixed(4);
  document.getElementById("eq-c2").textContent = metrics.coefs[2].toFixed(4);

  // update tabel koefisien
  const vars = ["pm10", "o3", "co"];
  const rows = document.querySelectorAll("#coef-tbody tr");
  rows.forEach((row, i) => {
    row.querySelector(".coef-val").textContent = metrics.coefs[i].toFixed(4);
    const pct = metrics.pct[i];
    row.querySelector(".pct-val").textContent  = pct.toFixed(1) + "%";
...
function applyFilters() {
  ...
  document.getElementById("tbl-info").textContent =
    `Menampilkan ${data.length} dari ${total} baris`;
...
async function updateCharts() {
  ...
  ["card-heatmap","card-pls-scores","card-pls-loadings",
   "card-scatter","card-avp","card-res"].forEach(setSpinner);

  // dim metric cards while loading
  document.querySelectorAll(".metric").forEach(m => m.classList.add("updating"));

  try {
    const res = await fetch("/charts", {
      method: "POST",
      headers: {"Content-Type":"application/json"},
      body: JSON.stringify({ rows, theme })
    });
    ...
    // ✅ UPDATE METRIK DINAMIS
    if (data.metrics) {
      updateMetricsDom(data.metrics);
    }
...
async function predict() {
  ...
  try {
    const res = await fetch("/predict", {
      method: "POST",
      headers: {"Content-Type":"application/json"},
      body: JSON.stringify({pm10, kategori, o3, co, critical, stasiun})
    });
    const data = await res.json();
...
function getMathExplanation(chartType, currentData) {
  ...
  if (n === 0) return { title:"Error", formula:"-", desc:"Tidak ada data." };
  let s_pm10=0, s_o3=0, s_co=0, s_ispu=0;
...
function openModal(chartType) {
  const cardMap = { heatmap:"card-heatmap", pls_scores:"card-pls-scores", pls_loadings:"card-pls-loadings", scatter:"card-scatter", avp:"card-avp", residual:"card-res" };
  const sourceImg = document.querySelector(`#${cardMap[chartType]} img`);
  document.getElementById("modal-img-target").innerHTML = sourceImg
    ? `<img src="${sourceImg.src}" alt="Grafik Detail">` : `<p style="color:var(--muted)">Memuat grafik...</p>`;
  const data = getMathExplanation(chartType, currentFiltered);
  document.getElementById("modal-title").innerHTML = data.title;
  document.getElementById("modal-formula").innerHTML = data.formula;
  document.getElementById("modal-explanation").innerHTML =
    `<div style="margin-bottom:12px;display:inline-block;background:rgba(255,103,29,0.15);color:var(--accent);padding:6px 12px;border-radius:8px;font-weight:700;font-size:0.9rem;font-family:'JetBrains Mono',monospace">Data Terfilter: n = ${currentFiltered.length} baris</div><br>` + data.desc;
...
def index():
    dm = default_metrics
    coef_rows = list(zip(["pm10", "o3", "co"], dm["coefs"], dm["pct"]))
    table_data = df_raw[["pm10","kategori","o3","co","critical","stasiun","ispu_max"]].to_dict("records")
    return render_template_string(
        HTML,
        def_r2=dm["r2"], def_r2_adj=dm["r2_adj"],
        def_rmse=dm["rmse"], def_mae=dm["mae"],
        def_intercept=dm["intercept"],
        def_coefs=dm["coefs"],
        def_coef_rows=coef_rows,
        n_total=len(df_raw),
...
def charts_route():
    try:
        ...
        if len(df_sub) < 5:
            return jsonify({"error": "Data terlalu sedikit (min 5 baris)."}), 400

        # ✅ Hitung metrik & split SATU KALI — grafik pakai hasil yang sama
        metrics = compute_ols_metrics(df_sub)
        yte   = metrics.pop("_yte")    # array numpy, jangan dikirim ke JSON
        ypred = metrics.pop("_ypred")

        return jsonify({
            "n":           len(df_sub),
            "metrics":     metrics,          # ← dikirim ke frontend
            ...
            # ← pakai yte/ypred/r2 dari compute_ols_metrics → R² pasti sama
            "act_vs_pred": make_actual_vs_pred(yte, ypred, metrics["r2"], is_dark),
            "residual":    make_residual(yte, ypred, len(df_sub), is_dark),
        })
...
def predict_route():
    ...
    is_kat_sedang      = int(kategori == "SEDANG")
    is_kat_tidak_sehat = int(kategori == "TIDAK SEHAT")
    is_kat_sangat      = int(kategori == "SANGAT TIDAK SEHAT")
    is_kat_bahaya      = int(kategori == "BERBAHAYA")
    is_crit_o3         = int(critical == "O3")
    is_crit_pm10       = int(critical == "PM10")
    is_crit_pm25       = int(critical == "PM2.5")
    is_crit_so2        = int(critical == "SO2")
    is_dki2            = int(stasiun == "DKI2 Kelapa Gading")
    is_dki3            = int(stasiun == "DKI3 Jagakarsa")
    is_dki4            = int(stasiun == "DKI4 Lubang Buaya")
    is_dki5            = int(stasiun == "DKI5 Kebon Jeruk")
    ...
    return jsonify({"prediction": round(max(0, pred), 2)})
